...
if [ $syncServer = "无" ]; then
    # 构建在本地
    echo ""
    echo "未设置同步服务器地址, 此步骤跳过~ "
else
    # 格式化 占位符
    echo ""
    # 执行 同步
    sh ${serverShell}/ci/sync.sh $distName $syncServer
fi

echo ""
echo "---------- 8、推送收包目录 ----------"

if [ $packageServer = "无" ]; then
    # 构建在本地
    echo ""
    echo "此次构建, 仅做服务器更新, 不进行 237 推送, 此步骤跳过~ "
else
    echo ""
    # 执行 推送
    sh ${serverShell}/ci/pull.sh $distName $packageServer
fi
